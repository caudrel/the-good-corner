...
export default function AdminCategories() {
  ...
  return (
    <AdminLayout title="gestion des categories - TGC">
      ...
      >
        <label htmlFor="name">
          Nouvelle Catégorie : <input type="text" id="name" name="name" className="input mr-2" required />
        </label>
      ...
      {categories?.length !== 0 && (
        <table className="table mt-4">
          ...
          <tbody>
            {categories?.map((c) => (
              <AdminCategoryRow key={c.id} handleDeleteCategory={handleDeleteCategory} category={c} />
            ))}
          </tbody>
        </table>
      )}
